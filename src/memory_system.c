                                                ...
                                                uint32_t tag)
{
    ...
    struct cache_line *each_line;
    for (int i = 0; i < cache_system->associativity; i++) {
        each_line = &cache_system->cache_lines[(cache_system->associativity * set_idx) + i]; 
        if(tag == each_line->tag){ //if no such element exist then it will go to null
            return each_line; 
        }
    }
    return NULL;
